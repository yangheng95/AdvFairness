...
class HuggingFaceModelWrapper(PyTorchModelWrapper):
    ...
    def get_grad(self, text_input):
        ...
        input_tuple.to(model_device)
        predictions = self.model(**input_tuple).logits

        try:
            labels = predictions.argmax(dim=1)
            loss = self.model(**input_tuple, labels=labels)[0]
        except TypeError:
            raise TypeError(
        ...
        output = {"ids": input_tuple["input_ids"], "gradient": grad}
