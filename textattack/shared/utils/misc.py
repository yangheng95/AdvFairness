...
def html_style_from_tuple(style_tuple):
    """Turns.
    ...
    """
    style_str = ""
    for key in style_tuple:
        style_str += key + ": " + style_tuple[key] + ";"
    return 'style="{}"'.format(style_str)


def html_table_from_rows(rows, title=None, header=None, style_tuple=None):
    # Stylize the container div.
    if style_tuple:
        table_html = "<div {}>".format(html_style_from_tuple(style_tuple))
    else:
        table_html = "<div>"
