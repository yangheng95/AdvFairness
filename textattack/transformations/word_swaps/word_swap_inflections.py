...
class WordSwapInflections(WordSwap):
    ...
    def _get_replacement_words(self, word, word_part_of_speech):
        ...
        replacement_inflections_tuple = lemminflect.getAllLemmas(word)

        # if dict is empty, there are no replacements for this word
        if not replacement_inflections_tuple:
            return []
        ...
        if lemminflect_pos in replacement_inflections_tuple:
            lemma = replacement_inflections_tuple[lemminflect_pos][0]
        else:
            lemma = random.choice(list(replacement_inflections_tuple.values()))[0]
