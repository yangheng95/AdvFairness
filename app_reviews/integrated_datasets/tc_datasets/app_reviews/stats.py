...
data = load_dataset("app_reviews")
negative_reviews = []
positive_reviews = []
dataset = data['train']
for i in range(len(dataset)):
    if dataset[i]['star'] < 4:
        negative_reviews.append([dataset[i]['package_name'], dataset[i]['review'], dataset[i]['star']])
    else:
        positive_reviews.append([dataset[i]['package_name'], dataset[i]['review'], dataset[i]['star']])
