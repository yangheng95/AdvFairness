...
def clean_dataset(dataset, attributes, centered):
    ...
    sens_tuple = {c: 1 if c in sens_cols else 0 for c in df.columns}
    X, sens_tuple = one_hot_code(X, sens_tuple)
    sens_names = [key for key in sens_tuple.keys() if sens_tuple[key] == 1]
    print(
...
def one_hot_code(df1, sens_tuple):
    cols = df1.columns
    for c in cols:
        if isinstance(df1[c][0], str):
            ...
            if n > 2:
                for i in range(n):
                    ...
                    df1[col_name] = col_i
                    sens_tuple[col_name] = sens_tuple[c]
                del sens_tuple[c]
            else:
                col_name = c
                col = [1 if el == unique_values[0] else 0 for el in column]
                df1[col_name] = col
    return df1, sens_tuple
