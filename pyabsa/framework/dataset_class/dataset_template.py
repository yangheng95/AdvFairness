...
class PyABSADataset(Dataset):
    ...
    Methods
        ...
        load_data_from_tuple(self, dataset_tuple, dataset_type, **kwargs): loads the dataset from a dictionary object containing the preprocessed data. dataset_tuple is the dictionary object, dataset_type is the type of the dataset to load, and additional keyword arguments can be passed to customize the loading behavior.
    ...
    def __init__(self, config, tokenizer, dataset_type, **kwargs):
        ...
        if (
                self.config.get("dataset_tuple")
                and dataset_type in self.config.dataset_tuple
                and self.config.dataset_tuple[dataset_type]
        ):
            self.load_data_from_tuple(
                config.dataset_tuple, dataset_type=dataset_type, **kwargs
            )
    ...
    def load_data_from_tuple(self, dataset_tuple, dataset_type, **kwargs):
        """
        Load the dataset from a dictionary.
        :param dataset_tuple: A dictionary containing the dataset.
        :param dataset_type: The type of the dataset, which can be "train", "dev", or "test".
        :param kwargs: Additional arguments for loading the dataset, such as "text_column", "aspect_column", "label_column", "separator", and "data_path".
        """
        data = []
        for text, aspect, label in zip(
                dataset_tuple[dataset_type][kwargs["text_column"]],
                dataset_tuple[dataset_type][kwargs["aspect_column"]],
                dataset_tuple[dataset_type][kwargs["label_column"]],
        ):
            data.append(
                {
                    "text": text,
