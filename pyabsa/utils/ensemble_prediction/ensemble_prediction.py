...
class VoteEnsemblePredictor:
    ...
    def __ensemble(self, result: dict):
        ...
        if isinstance(result, dict):
            return self.__tuple_aggregate(result)
        elif isinstance(result, list):
            return self.__list_aggregate(result)
        else:
            return result

    def __tuple_aggregate(self, result: dict):
        """
        ...
        for k, v in result.items():
            if isinstance(result[k], list):
                ...
                ensemble_result[k] = self.__tuple_aggregate(result[k])
            else:
    ...
    def __list_aggregate(self, result: list):
        ...
        if isinstance(result[0], list):
            ...
            for k in result:
                result[k] = self.__tuple_aggregate(result[k])
            return result
